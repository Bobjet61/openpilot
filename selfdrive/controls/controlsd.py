...
class Controls:
  ...
  def update_events(self, CS):
    ...
    for i, pandaState in enumerate(self.sm['pandaStates']):
      ...
      controls_mismatch = (safety_mismatch and self.sm.frame*DT_CTRL > 10.) or \
                          pandaState.safetyRxChecksInvalid or self.mismatch_counter >= 200
      if controls_mismatch:
        # Diagnostic only: identify which independent condition caused the red Controls Mismatch alert.
        # Throttled to one record per second so the route log remains usable.
        if self.sm.frame % int(1. / DT_CTRL) == 0:
          ...
          cloudlog.error(
            ...
            f"controls_allowed={pandaState.controlsAllowed} "
            f"actual_model={pandaState.safetyModel} actual_param={pandaState.safetyParam} "
            f"actual_alt_exp={pandaState.alternativeExperience} "
            f"expected_model={getattr(expected, 'safetyModel', None)} "
